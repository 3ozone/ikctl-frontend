...
import { getServers, getKits, getGroups, createPipeline } from "@/lib/services";
import type { Server, Kit, Group } from "@/types";
...
function valuesFromKit(kit: Kit): { key: string; value: string }[] {
  return Object.entries(kit.values ?? {}).map(([key, val]) => ({
    key,
    value: String(val ?? ""),
  }));
}
...
export default function NewPipelinePage() {
  ...
  const [kitRows, setKitRows] = useState<KitRow[]>([emptyKitRow()]);
  ...
  const updateKitValue = (uid: number, idx: number, value: string) => {
    setKitRows((prev) =>
      prev.map((r) => {
        ...
        const values = r.values.map((v, i) => (i === idx ? { ...v, value } : v));
        return { ...r, values };
      })
    );
  };
      ...
      const pipeline = await createPipeline({
        ...
        kits: filledKits.map((r) => ({
          ...
          values: Object.fromEntries(
            r.values.filter((v) => v.key.trim()).map((v) => [v.key, v.value])
          ),
        })),
      });
      router.push(`/dashboard/pipelines/${pipeline.pipeline_id}`);
    } catch (err: unknown) {
      const e = err as { response?: { data?: { message?: string } } };
      setSubmitError(e?.response?.data?.message ?? "Failed to create pipeline.");
    }
  };
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      ...
      <div className="flex items-center gap-4">
        <Link href="/dashboard/pipelines" className="text-slate-400 hover:text-slate-600">
          <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
          </svg>
        </Link>
        <div>
          <h1 className="text-2xl font-bold text-slate-900">New pipeline</h1>
          <p className="text-sm text-slate-500">Run a sequence of kits across multiple servers.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <Card>
          <CardHeader>
            ...
            <p className="text-xs text-slate-400">Inherited by kits that don&apos;t set their own value.</p>
          </CardHeader>
          <CardBody className="space-y-4">
            ...
            <div className="flex flex-col gap-1">
              ...
              <select
                className="rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500 w-40"
        ...
        <Card>
          ...
          <CardBody className="space-y-3">
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="space-y-1">
                    ...
                    {row.sudo_override && (
                      <select
                        className="mt-1 w-full rounded border border-slate-300 bg-white px-2 py-1 text-sm outline-none focus:border-blue-500"
                      ...
                      {row.values.map((v, i) => (
                        <div key={v.key} className="flex items-center gap-2">
                          ...
                          <input
                            ...
                            className="flex-1 rounded border border-slate-300 bg-white px-2 py-1 text-sm outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                          />
                        </div>
                      ))}
                    </div>
            ...
            <button
              type="button"
              onClick={addKitRow}
              className="flex w-full items-center justify-center gap-2 rounded-lg border-2 border-dashed border-slate-200 py-2 text-sm text-slate-400 hover:border-blue-300 hover:text-blue-500 transition"
            >
              <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
